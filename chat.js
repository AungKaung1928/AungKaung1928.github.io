...
const TOPICS = [
    ...
    },
    {
        ...
        a: "## SO-ARM100 Manipulation with RL, Imitation and Language — Python, MuJoCo, PyTorch, LeRobot\n" +
           ...
           "Status, stated plainly: all four repositories are code-complete with 127 tests green. The bench reference is measured: at 100 episodes times 5 seeds the scripted expert scores 0.93 on lift and pick-and-place, but 0.73 and 0.72 on the small cube, the one physics setting that breaks it. Reinforcement learning is measured, on one training seed: PPO lifts the cube from scratch at 1.0 million steps; the same recipe under full randomisation stalled, rolling success never above 0.09, and was stopped at 4.9 million; a randomised fine-tune of the nominal policy halves the mean success drop on the six held-out physics settings, 0.120 to 0.056, almost all of it on 3 steps of action latency (0.46 to 0.76). The randomised policy also had 10 million more training steps, so part of that gain may be training length; the control run has not been done. Language is measured, on one training seed, and it is weak: 0.11 success on seen instructions, 0.10 on paraphrases, 0.03 on unseen task-colour pairs; the encoder ablation (MiniLM, hashed, no FiLM) sits inside its own noise, because held-out demonstration L1 is 0.128 against 0.057 on training ones and the fix is more demonstrations, not another encoder. SmolVLA-base zero-shot scores 0 of 40 and needs 3.2 s per 50-step action chunk on CPU. The imitation tables are not measured yet.\n\n" +
        ...
        deep: "## SO-ARM100 Manipulation with RL, Imitation and Language — Python, MuJoCo, PyTorch, LeRobot\n" +
           ...
           "127 tests (63, 21, 19 and 24) green across the four repositories. The latency off-by-one, found and fixed. One measurement that changed a plan: MiniLM cosine similarities on the instruction templates are dominated by surface words — lift green against lift blue scores 0.82 while a true paraphrase scores 0.59 — so paraphrase generalisation cannot be assumed from the encoder and has to be tested. The LoRA gate at 0.51 s per step. The expert's reference table, 100 episodes times 5 seeds per cell: reach 1.00 everywhere; lift 0.93 nominal, 0.90 heavy, 0.97 weak, 0.73 small; pick-and-place 0.93 nominal, 0.85 heavy, 0.72 small; push 0.73 to 0.79 in every cell, a limit of the scripted push itself. The one-seed development readings had put the small cube at 0.87 and 0.80, which is why they were never the reference. Throughput is 1,954 environment steps per second on one process and 7,302 on eight in 15-second bursts, but 5,361 sustained; the first three sweeps were discarded for drift until a warm-up was added to get past the Windows power plan's low starting clock. RL: Reinforcement learning is measured, on one training seed: PPO lifts the cube from scratch at 1.0 million steps; the same recipe under full randomisation stalled, rolling success never above 0.09, and was stopped at 4.9 million; a randomised fine-tune of the nominal policy halves the mean success drop on the six held-out physics settings, 0.120 to 0.056, almost all of it on 3 steps of action latency (0.46 to 0.76). The randomised policy also had 10 million more training steps, so part of that gain may be training length; the control run has not been done. The ONNX export of that policy matches PyTorch to 2.1e-6 and runs in 0.008 ms on one thread. Along the way two more training failures were caught and kept: KL blow-up without a stop (clip fraction 0.77, sigma collapsed to 0.054), and a hover exploit where the policy held the cube just under the 5 cm success line because success ended the episode. Language is measured, on one training seed, and it is weak: 0.11 success on seen instructions, 0.10 on paraphrases, 0.03 on unseen task-colour pairs; the encoder ablation (MiniLM, hashed, no FiLM) sits inside its own noise, because held-out demonstration L1 is 0.128 against 0.057 on training ones and the fix is more demonstrations, not another encoder. SmolVLA-base zero-shot scores 0 of 40 and needs 3.2 s per 50-step action chunk on CPU. The imitation tables are still marked not yet measured, each with the command that produces it.\n" +
           "## Limits\n" +
    ...
    },
    {
        ...
        a: "Four current projects, all robot learning on a CPU, and four earlier ones they grew out of.\n\n" +
           ...
           "3. SO-ARM100 Manipulation with RL, Imitation and Language — MuJoCo, PyTorch, LeRobot. One bench, three ways to learn the same arm; 127 tests, bench reference and RL measured (randomised fine-tune halves the held-out drop, mostly on latency), language measured and weak (0.11 seen, 0.03 unseen pairs), imitation tables not yet measured.\n" +
        ...
        deep: "Four current projects and four earlier ones. All of the current work is robot learning, trained and measured on CPU only.\n\n" +
           ...
           "- 127 tests green; the expert reference is measured (0.93 lift, 0.73 on the small cube); RL is measured on one seed (held-out drop 0.120 to 0.056 with a randomised fine-tune, almost all on action latency); language is measured and weak (0.11 seen, 0.03 on unseen pairs, ablation inside noise); imitation is not yet measured, as the page says\n" +
    ...
    },
    {
        ...
        a: "## Published on this page\n" +
           ...
           "- 127 tests green across four repositories, the expert reference and the RL transfer table measured (one seed), language measured and weak, imitation marked not yet measured — SO-ARM100 Manipulation\n\n" +
           "## Not published\n" +
    ...
    },
    {
        ...
        a: "Straight answer, because the question deserves one.\n\n" +
           ...
           "- The SO-ARM100 imitation tables are not measured yet, the language policy is measured and weak, and the one sim-to-real method result that is measured, Microduck domain randomisation, came out negative\n" +
        ...
        deep: "## The gaps, plainly\n" +
           ...
           "- One of the four current cards is ahead of its numbers: the SO-ARM100 imitation tables are marked not yet measured (the bench reference, the RL table and the language table are, each on a single training seed; the language policy is weak). The Microduck policies are the first half of their schedule on one seed; its domain-randomisation result is measured and negative\n" +
